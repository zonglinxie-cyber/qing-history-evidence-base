...
// 结构层（required / minCount / unique / era 枚举，均 manifest 驱动）在 validate-data.mjs，朝代无关。
// 本文件只放清史专属检查；新朝代写自己的 rules/<dynasty>.mjs，不继承这些不变量。
// 待办：画像/今地权利一致性、FK、章节链接解析等"通用但尚未上提"的检查暂留于此，后续可抽到共享层。
...
export function check(ctx) {
  const {
    ...
    princes, princesses, heirChain, historicSites,
  } = ctx;
  ...
  // 章节：人物归属、来源单元引用、相关链接解析；必须含康熙/雍正即位章；每帝有章
  ...
  for (const row of chapters) {
    ...
    for (const href of String(row.related || '').split(/[；;]/).map((item) => item.trim()).filter(Boolean)) {
      const sId = href.match(/^#\/site\/(QH-ST-\d+)$/)?.[1];
      const lId = href.match(/^#\/lane\/(QH-L-\d+)$/)?.[1];
      const pId = href.match(/^#\/person\/(QH-P-\d+)$/)?.[1];
